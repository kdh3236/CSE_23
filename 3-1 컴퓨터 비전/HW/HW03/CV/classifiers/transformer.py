...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        caption = self.embedding(captions)
        caption = self.positional_encoding(caption)

        mask = torch.tril(torch.ones((T, T)))
        
        feature = self.visual_projection(features)
        feature = feature.reshape(N, 1, -1).to(dtype=torch.float32) # (N, 1, D)


        transformer_output = self.transformer(caption, feature, mask)
