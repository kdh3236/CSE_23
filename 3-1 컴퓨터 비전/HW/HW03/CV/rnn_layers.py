...
def rnn_step_backward(dnext_h, cache):
    ...
    x, prev_h, Wx, Wh, s, b = cache

    ## Local derivative
    dLds = dnext_h * (1-(np.tanh(s)**2))

    dx = dLds.dot(Wx.T)
    dprev_h = dLds.dot(Wh.T)
    dWx = x.T.dot(dLds)
    dWh = prev_h.T.dot(dLds)
    db =  dLds.sum(axis = 0) # mini - batch라 그냥 dLds가 아니라 N sample만큼 더해야함
...
def rnn_forward(x, h0, Wx, Wh, b):
    ...
    # 전체 파라미터를 배열 안 튜플로 저장
    cache = []
    h = np.zeros((N, T, H))

    for i in range(T):
        if (i == 0): h_temp, cache_temp = rnn_step_forward (x[:, i, :], h0, Wx, Wh, b);
        else:  h_temp, cache_temp = rnn_step_forward (x[:, i, :], h[:, i-1, :], Wx, Wh, b);
...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
        dx_temp, dh_temp, dWx_temp, dWh_temp, db_temp = rnn_step_backward(dh_temp + dh[:, i, :], cache[i])
...
def lstm_step_forward(x, prev_h, prev_c, Wx, Wh, b):
    ...
    i_t = sigmoid(x@Wx[:, 0:H] + prev_h@Wh[:, 0: H] + b[0:H]) # N*H
    f_t = sigmoid(x@Wx[:, H:2*H] + prev_h@Wh[:, H: 2*H] + b[H:2*H]) # N*H
    o_t = sigmoid(x@Wx[:, 2*H:3*H] + prev_h@Wh[:, 2*H: 3*H] + b[2*H:3*H]) # N*H
    g_t = np.tanh(x@Wx[:, 3*H:4*H] + prev_h@Wh[:, 3*H: 4*H] + b[3*H:4*H]) # N*H

    next_c = f_t*prev_c + i_t*g_t # N*H
    next_h = o_t*next_c 

    cache = (x, prev_h, prev_c, next_c, i_t, f_t, o_t, g_t, b, Wh, Wx)
...
def lstm_step_backward(dnext_h, dnext_c, cache):
    ...
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    ## i_t ~ o_t : sigmoid 결과
    x, prev_h, prev_c, next_c, i_t, f_t, o_t, g_t, b, Wh, Wx = cache

    dc = dnext_h * o_t + dnext_c
...
def lstm_forward(x, h0, Wx, Wh, b):
    ...
    for i in range(T):
        if (i == 0): h_temp, c_temp, cache_temp = lstm_step_forward(x[:, i, :], h0, c, Wx, Wh, b);
        else:  h_temp, c_temp, cache_temp = lstm_step_forward(x[:, i, :], h[:, i-1, :], c, Wx, Wh, b);
...
def lstm_backward(dh, cache):
    ...
    for t in reversed(range(T)):
        dx_temp, dprev_h, dprev_c, dWx_temp, dWh_temp, db_temp = lstm_step_backward(dh[:, t, :] + dprev_h, dprev_c, cache[t])
